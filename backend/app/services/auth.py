...
class AuthService:
    ...
    async def resolve_user(self, provider: AuthProvider, result: AuthResult) -> User:
        ...
        await self.user_service.audit(user=user, event="login_success", payload={"provider": provider.name})
    ...
    async def issue_tokens(self, user: User, provider: AuthProvider, *, client_info: str | None = None) -> TokenBundle:
